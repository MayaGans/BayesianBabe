...
I assume recursive functions are also possible with R - I wonder how one would turn this dataframe into a nested list? 
...
library(tidyr)
anatomy <- tibble(
  id = c('body', 'hand', 'ring finger', 'pinky', 'foot', 'big toe'),
  parent = c(NA, 'body', 'hand', 'hand', 'body', 'foot')
)
```
...
```{r}
list(body =
 list(hand = list(c(list(`ring finger` = list(), pinky = list())))),
 list(foot = list(`big toe` = list()))
)
```
