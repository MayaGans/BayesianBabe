...
Creating the generator code is easy! Now that I'm just making strings I feel pretty optimistic about including blocks for any data transformation needs. My second week success is I now have a correct Blockly mental model which is a pretty big one. However, in true coder (read perpetual student) fashion, this leaves me with another challenge: if the block code cannot access the javascript how do I make my drop down menus dynamic? I don't want to have a filter block that ONLY applies to the iris dataset, and another that applies for mtcars? That wouldn't scale to users inputting their own data at all. More to come!